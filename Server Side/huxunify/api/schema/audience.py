...
from marshmallow.fields import Str, Integer, String
...
class AudienceDeliverySchema(Schema):
    ...
    class Meta:
        """expose the fields for serialization"""
        # Fields to expose
        fields = ["delivery_platform_id", "audience_delivery_status", "audience_id", "completed", "count", "delivery_job_id", "delivery_job_status", "created"]
...
class AudienceInsightsSchema(Schema):
    ...
    """
    # define parameters
    insights = List(Dict(), required=True, validate=Length(max=1000),
                       example={
                           "age": {
                               "breakdown": {
                                   "additionalProp1": 0,
