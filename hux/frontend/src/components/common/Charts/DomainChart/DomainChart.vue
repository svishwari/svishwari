<template>
  <div class="chart-container" :style="{ maxWidth: chartWidth }">
    <div ref="domainChart" class="chart-section"></div>
  </div>
</template>
...
export default {
  ...
  watch: {
    chartDimensions: {
      handler() {
        d3Select.select(this.$refs.domainChart).selectAll("svg").remove()
  ...
  methods: {
    ...
    async initiatelineAreaChart() {
      ...
      let appendyAxisFormat
      if (this.typeOfChart == "sent") {
        appendyAxisFormat = (text) => this.applyFilter(text, "numeric")
      } else {
        appendyAxisFormat = (text) => this.applyFilter(text, "percentage")
      }
