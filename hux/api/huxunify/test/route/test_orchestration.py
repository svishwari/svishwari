...
class OrchestrationRouteTest(RouteTestCase):
    ...
    def setUp(self) -> None:
        ...
        for subclass in CloudClient.__subclasses__():
            ...
            mock.patch.object(subclass, "get_secret", return_value="secret").start()
        ...
        audiences = [
            {
                ...
                db_c.SIZE: 3329,
                "audience_tags": {api_c.INDUSTRY: [api_c.HEALTHCARE, api_c.RETAIL]},
            },
            {
    ...
    def test_create_audience_with_destination(self):
        ...
        self.assertEqual(response.json[db_c.TAGS], {db_c.INDUSTRY: [api_c.HEALTHCARE]})
    ...
    def test_get_audience(self):
        ...
        for standalone_delivery in audience[api_c.AUDIENCE_STANDALONE_DELIVERIES]:
            self.assertIn(
    ...
    def test_get_audience_with_contact_preference(self):
        ...
        audience = response.json
        self.assertEqual(HTTPStatus.OK, response.status_code)
        self.assertEqual(ObjectId(audience[db_c.OBJECT_ID]), self.audiences[2][db_c.ID])
        self.assertEqual(audience[db_c.CREATED_BY], self.user_name)
    ...
    def test_get_lookalike_audience(self):
        ...
        response = self.app.get(
            f"{self.audience_api_endpoint}/" f"{self.lookalike_audience_doc[db_c.ID]}",
            headers=t_c.STANDARD_HEADERS,
        )
        ...
        self.assertEqual(self.lookalike_audience_doc[api_c.NAME], audience[api_c.NAME])
        self.assertEqual(
    ...
    def test_get_audiences_with_valid_filters(self):
        ...
        self.assertIn(api_c.HEALTHCARE, audiences[0][api_c.TAGS][api_c.INDUSTRY])
        self.assertIn(api_c.TAGS, audiences[1])
        self.assertIn(api_c.HEALTHCARE, audiences[1][api_c.TAGS][api_c.INDUSTRY])
    ...
    def test_get_audiences_with_contact_preference_attribute_filters(self):
        ...
        self.assertEqual(str(self.audiences[2][db_c.ID]), audiences[0][api_c.ID])
        self.assertIn(api_c.TAGS, audiences[0])
        self.assertIn(api_c.AUTOMOTIVE, audiences[0][api_c.TAGS][api_c.INDUSTRY])
    ...
    def test_viewer_user_permissions(self) -> None:
        ...
        delete_user(self.database, t_c.VALID_INTROSPECTION_RESPONSE.get(api_c.OKTA_UID))
