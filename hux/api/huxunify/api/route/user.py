...
class UserProfile(SwaggerView):
    ...
    def get(self, user: dict) -> Tuple[dict, int]:
        ...
        try:
            ...
            config = get_config()
            show_latest_release_notes = config.RELEASE_VERSION_LATEST != user.get(
...
class SeenNotifications(SwaggerView):
    ...
    def get(
        ...
        return HuxResponse.OK(
            data={db_c.SEEN_NOTIFICATIONS: user.get(db_c.SEEN_NOTIFICATIONS, False)}
        )
...
class UsersRBACMatrix(SwaggerView):
    ...
    def get(self, user: dict) -> Tuple[dict, int]:
        ...
        if not rbac_matrix:
            logger.error("RBAC matrix document not found in configurations collection.")
