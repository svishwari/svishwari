...
def get_email_insights(
    token: str,
    filters: Optional[dict] = api_c.CUSTOMER_OVERVIEW_DEFAULT_FILTER,
) -> list:
    """Get customer insights by email.

    Args:
        token (str): OKTA JWT Token.
        filters (dict):  filters to pass into email_domains endpoint,
            default None.

    Returns:
        list: list of customer insights by email.
    """

    config = get_config()
    logger.info("Retrieving customer insights by email domains.")
    response = requests.post(
        f"{config.CDP_SERVICE}/customer-profiles/insights/email-domains",
        json=filters if filters else {},
        headers={
            api_c.CUSTOMERS_API_HEADER_KEY: token,
        },
    ...
    if response.status_code != 200 or api_c.BODY not in response.json():
        logger.error(
            "Failed to customer insights by email domains. %s %s.",
            response.status_code,
            response.text,
        )
        raise iae.FailedAPIDependencyError(
    ...
    logger.info("Successfully retrieved email insights.")
