...
describe("Tests Audience", () => {
    ...
    it("should be able to view Audiences List and Dashboard", () => {
        ...
        cy.get(selector.audience.audiencenameclick).eq(0).find("a").should('have.attr', 'href')
            .then((href) => {
                cy.visit(href)
            })
    ...
    it("should be able to validate engagements and delivery", () => {
        ...
        cy.get("@overviewListCount").then(() => {
            cy.get(selector.audience.engagementdelivery).its("length").should("be.gt", 0)
        })
    })

    it("should be able to validate Audiences Overview", () => {
        cy.get(selector.audience.overview)
            .its("length")
            .as("overviewListCount")
    ...
    it("should be able to view Gender / monthly spending chart", () => {
        ...
        cy.get(selector.audience.genderSpendChart)
            .its("length")
            .should("gt", 0)
    })
    ...
    it("should be able to open and validate deliver history", () => {
        ...
        cy.get("@deliveryhistory").then(() => {
            cy.get(selector.audience.deliveryhistorydrawer).its("length").should("be.gt", 0)
        })
    })
