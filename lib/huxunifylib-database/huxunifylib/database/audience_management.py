...
def create_audience(
    ...
    if ingestion_job_id:
        audience_doc[db_c.JOB_ID] = ingestion_job_id
...
def get_ingested_data(
    ...
    data_collection = dm_db[db_c.INGESTED_DATA_COLLECTION]
    ...
    try:
        ...
        for count, item in enumerate(cursor):
            ingested_data = item[db_c.INGESTED_DATA]
...
def get_audience(
    ...
    try:
        doc = collection.find_one({db_c.ID: audience_id, db_c.DELETED: False})
    except pymongo.errors.OperationFailure as exc:
        logging.error(exc)
    ...
    ingestion_job_id = doc[db_c.JOB_ID]
    audience_filters = doc.get(db_c.AUDIENCE_FILTERS, [])

    # Initialize queries list
    filter_queries = [{db_c.JOB_ID: ingestion_job_id}]

    # Apply start_id is not None
    if start_id is not None and ObjectId.is_valid(start_id):
        filter_queries.append({db_c.ID: {"$gt": start_id}})

    # Loop through the filters and apply them
    for item in audience_filters:
        if db_c.AUDIENCE_FILTER_FIELD not in item.keys():
            logging.error(
                "Expected %s in audience filter! Skipping the filter!",
                db_c.AUDIENCE_FILTER_FIELD,
            )
            continue

        if db_c.AUDIENCE_FILTER_TYPE not in item.keys():
            logging.error(
                "Expected %s in audience filter! Skipping the filter!",
                db_c.AUDIENCE_FILTER_TYPE,
            )
            continue

        if db_c.AUDIENCE_FILTER_VALUE not in item.keys():
            logging.error(
                "Expected %s in audience filter! Skipping the filter!",
                db_c.AUDIENCE_FILTER_VALUE,
            )
            continue
        ...
        if filter_type in [db_c.AUDIENCE_FILTER_MIN, db_c.AUDIENCE_FILTER_MAX]:
            if not isinstance(filter_value, (float, int)):
                raise de.IncorrectFilterValue(filter_type, filter_value)
        elif filter_type in [
            db_c.AUDIENCE_FILTER_INCLUDE,
            db_c.AUDIENCE_FILTER_EXCLUDE,
        ]:

            if not isinstance(filter_value, list):
                filter_value = [filter_value]
        elif filter_type == db_c.AUDIENCE_FILTER_EXISTS:
            if not isinstance(filter_value, bool):
                raise de.IncorrectFilterValue(filter_type, filter_value)

        if filter_type == db_c.AUDIENCE_FILTER_MIN:
            filter_queries.append({filter_field: {"$gte": filter_value}})
        elif filter_type == db_c.AUDIENCE_FILTER_MAX:
            filter_queries.append({filter_field: {"$lte": filter_value}})
        elif filter_type == db_c.AUDIENCE_FILTER_INCLUDE:
            filter_queries.append({filter_field: {"$in": filter_value}})
        elif filter_type == db_c.AUDIENCE_FILTER_EXCLUDE:
...
def get_audience_insights(
    ...
    dm_db = database[db_c.DATA_MANAGEMENT_DATABASE]
    collection = dm_db[db_c.AUDIENCE_INSIGHTS_COLLECTION]

    try:
        doc = collection.find_one({db_c.AUDIENCE_ID: audience_id})
    except pymongo.errors.OperationFailure as exc:
        logging.error(exc)
...
def get_audience_data_source_id(
    ...
    if audience_doc is not None:
        ingestion_job_doc = dm.get_ingestion_job(
            database,
            audience_doc[db_c.JOB_ID],
        )

    if ingestion_job_doc is not None:
        data_source_id = ingestion_job_doc[db_c.DATA_SOURCE_ID]
...
def append_audience_insights(
    ...
    try:
        ...
        collection.create_index([(db_c.AUDIENCE_ID, pymongo.ASCENDING)])
...
def delete_audience_insights(
    ...
    success_flag = False
    am_db = database[db_c.DATA_MANAGEMENT_DATABASE]
    collection = am_db[db_c.AUDIENCE_INSIGHTS_COLLECTION]

    try:
        collection.delete_one({db_c.AUDIENCE_ID: audience_id})
        success_flag = True
    except pymongo.errors.OperationFailure as exc:
        logging.error(exc)
...
def get_audience_name(
    ...
    if doc is not None and db_c.AUDIENCE_NAME in doc:
        audience_name = doc[db_c.AUDIENCE_NAME]
...
def update_audience_name(
    ...
    if exists_flag:
        ...
        if cur_doc[db_c.AUDIENCE_NAME] == name:
            raise de.DuplicateName(name)
...
def get_ingestion_job_audience_ids(
    ...
    audience_ids = [doc[db_c.ID] for doc in cursor]
...
def get_audiences_count(database: DatabaseClient) -> int:
    ...
    return get_collection_count(
        database, db_c.DATA_MANAGEMENT_DATABASE, db_c.AUDIENCES_COLLECTION
    )
